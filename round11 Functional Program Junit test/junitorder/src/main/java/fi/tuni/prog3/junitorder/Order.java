...
/**
 * @author Tuan Anh Nguyen anh.5.nguyen@tuni.fi Represents an order that
...
public class Order {
     ...
     * @throws IllegalArgumentException if the item unit count to add is not
     * positive.
     * @throws IllegalStateException if an existing entry has the same item name
     * but a different price than the added item.
     */
   public boolean addItems(Item item, int count) throws IllegalArgumentException, IllegalStateException {
    ...
    for (Entry entry : entries) {
        if (entry.getItem().getName().equals(item.getName())) {
            ...
            entry.count += count; // accessing the package-private field directly
            return true;
        }
    }
     ...
     * @throws IllegalArgumentException if the item unit count to add is not
     * positive.
     * @throws NoSuchElementException if the order does not contain an entry
    ...
    /**
     * Returns the total number of entries in this order (sum of all entries
     * counts).
     *
     ...
     * @throws IllegalArgumentException if the item unit count to remove is not
     * positive or is larger than the corresponding existing entry's item unit
     * count.
     * @throws NoSuchElementException if the order does not contain an entry
     * with the specified item name.
     */
    public boolean removeItems(String name, int count) throws IllegalArgumentException, NoSuchElementException {
        for (int i = 0; i < entries.size(); i++) {
            ...
            if (entry.getItemName().equals(name)) {
                ...
                int newCount = entry.getCount() - count;

                // Remove the entry from the list if its count becomes 0
                if (newCount == 0) {
                    entries.remove(i);
                } else {
                    entries.set(i, new Entry(entry.getItem(), newCount));
                }
                return true;
            }
        }
    ...
    /**
     * Represents an order entry that consists of an item and an item unit
     * count.
     */
    public static class Entry {
        ...
        /**
         * Returns a string representation of this entry. The presentation
         * format is "x units of item", where x is the item unit count and item
         * is the string representation of this entry's item.
         *
    ...
    public static class Item {
        ...
        /**
         *
         * Returns a string representation of this item. The presentation format
         * is "Item(name, price)", where the price is expressed with two
         ...
         * @return true if the name of this item equals the name of the other
